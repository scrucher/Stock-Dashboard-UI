...
const StockTable = ({ path }) => {
  ...
  const handleEdit = async (updatedItem) => {
    try {
      ...
      setListAll(
        GetAll.map((item) => (item.id === updatedItem.id ? updatedItem : item))
      );
  ...
  return (
    <div className="p-4">
          ...
          <table className="w-full table-fixed rounded-2xl bg-gray-100">
            <thead className="rounded-3xl bg-gray-200">
              <tr>
                ...
                <th className="p-2 text-left w-1/7">Product Name</th>
                ...
                <th className="p-2 text-left w-1/7">Quantity</th>
              ...
              <tbody>
                {path.map((data, index) => (
                  <tr key={index} className="border-t text-left">
                    ...
                    <td className="p-2 w-1/7">{data.quantity}</td>
                    <td className="p-2 w-1/7">
      ...
      >
        <p>
          This will permanently delete the data. This action cannot be undone.
        </p>
        <div className="mt-4 text-right">
          <Button
            className="mr-2 bg-red-500 hover:bg-red-300"
